      ...
      <div class="am-u-sm-12">
        <form class="am-form">
              ...
              <tr>
                <th class="table-check"><input type="checkbox" id="allCheckbox" /></th>
                 <th class="table-check">ID</th>
						...
						<g:sortableColumn property="xz" title="${message(code: 'priceRule.xz.label', default: 'Xz')}" />
					
						<g:sortableColumn property="price" title="${message(code: 'priceRule.price.label', default: 'Price')}" />
					
						<g:sortableColumn property="factor1" title="${message(code: 'priceRule.factor1.label', default: 'Factor1')}" />
					
						<g:sortableColumn property="factor2" title="${message(code: 'priceRule.factor2.label', default: 'Factor2')}" />
          ...
          <tbody>
            ...
            <tr>
            	<g:each in="${priceRuleInstanceList}" status="i" var="priceRuleInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
					<td><input type="checkbox" name="checkbox" value="${priceRuleInstance.id}" /></td>
					<td>${priceRuleInstance.id}</td>
					
						<td><g:link action="show" id="${priceRuleInstance.id}">${fieldValue(bean: priceRuleInstance, field: "xz")}</g:link></td>
					
						<td>${fieldValue(bean: priceRuleInstance, field: "price")}</td>
					
						<td>${fieldValue(bean: priceRuleInstance, field: "factor1")}</td>
					
						<td>${fieldValue(bean: priceRuleInstance, field: "factor2")}</td>
          ...
          <div class="am-cf">
			  ...
			  共${priceRuleInstanceCount ?: 0}条记录
			  <div class="am-fr">
			    <ul class="am-pagination pagination">
			        <g:paginate next="下一页" prev="上一页"
		            maxsteps="0" controller="PriceRule"
		            action="list" total="${priceRuleInstanceCount ?: 0}" />
